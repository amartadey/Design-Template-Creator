...
function attachEventListeners() {
    ...
    document.getElementById('bulkImageInput').addEventListener('change', handleBulkImageSelection);
    document.getElementById('confirmBulkUpload').addEventListener('click', createPagesFromBulkUpload);

    // Drag and drop for bulk upload zone
    const bulkUploadZone = document.getElementById('bulkUploadZone');
    bulkUploadZone.addEventListener('click', () => {
        document.getElementById('bulkImageInput').click();
    });
    bulkUploadZone.addEventListener('dragenter', handleBulkDragEnter);
...
function resetBulkUpload() {
    ...
    document.getElementById('bulkUploadZone').style.display = 'flex';
    document.getElementById('bulkPreviewGrid').style.display = 'none';
    document.getElementById('previewItems').innerHTML = '';
    document.getElementById('imageCount').textContent = '0';
    document.getElementById('confirmBulkUpload').style.display = 'none';
}
...
function handleBulkImageSelection(e) {
...
function processBulkImages(files) {
    bulkUploadImages = [];
    ...
    files.forEach((file, index) => {
        ...
        reader.onload = (e) => {
            ...
            // Generate page name: replace hyphens with spaces, capitalize first letter of each word
            const pageName = filenameWithoutExt
            ...
            previewItem.dataset.index = bulkUploadImages.length - 1;
            previewItem.innerHTML = `
                <img src="${imageData}" alt="${pageName}">
                <div class="preview-item-info">
                    <div class="preview-item-name">${originalFilename}</div>
                    <div class="preview-item-page">→ ${pageName}</div>
                </div>
                <button class="preview-item-remove" onclick="removeBulkImage(${bulkUploadImages.length - 1})" title="Remove">×</button>
            `;
            ...
            if (processedCount === files.length) {
                ...
                document.getElementById('bulkPreviewGrid').style.display = 'block';
                document.getElementById('imageCount').textContent = bulkUploadImages.length;
                document.getElementById('confirmBulkUpload').style.display = 'inline-flex';
            }
        };
...
function removeBulkImage(index) {
    bulkUploadImages.splice(index, 1);
    ...
    bulkUploadImages.forEach((img, idx) => {
        const previewItem = document.createElement('div');
        previewItem.className = 'preview-item';
        previewItem.dataset.index = idx;
        previewItem.innerHTML = `
            <img src="${img.imageData}" alt="${img.pageName}">
            <div class="preview-item-info">
                <div class="preview-item-name">${img.originalFilename}</div>
                <div class="preview-item-page">→ ${img.pageName}</div>
            </div>
            <button class="preview-item-remove" onclick="removeBulkImage(${idx})" title="Remove">×</button>
        `;
        previewItems.appendChild(previewItem);
    });
...
function createPagesFromBulkUpload() {
    if (bulkUploadImages.length === 0) {
    ...
    bulkUploadImages.forEach(img => {
        // Check for duplicate slug
        let finalSlug = img.slug;
        let counter = 1;
        while (state.pages.some(p => p.slug === finalSlug)) {
            finalSlug = `${img.slug}-${counter}`;
            counter++;
        }
    ...
    const originalHTML = btn.innerHTML;
    btn.innerHTML = '<span class="icon">✓</span> ' + bulkUploadImages.length + ' Pages Created!';
    setTimeout(() => {
        btn.innerHTML = originalHTML;
    }, 3000);
}
